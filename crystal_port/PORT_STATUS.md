...
- minimal terrain fill workflow with per-hex color overrides, translucent hover preview, left-drag paint, right-drag clear, adjustable fill radius, inspector fill color control plus preset swatches, and Crystal slice JSON persistence
...
- fill-layer editing parity beyond a single active fill color with preset swatches and adjustable radius, per-hex color paint/erase, and slice-state persistence
