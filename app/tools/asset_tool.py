...
from PySide6.QtGui import (
    QColor, QImage, QKeyEvent, QMouseEvent, QPainter, QPen, QPixmap,
)
...
class AssetTool(Tool):
    def __init__(self, project: Project, command_stack: CommandStack):
        ...
        # Erase size drag state (Alt+drag vertical)
        ...
        # Restore stroke state (Ctrl+drag in erase mode)
        self._restore_stroke_active: bool = False
        self._restore_pre_snapshot = None  # QImage snapshot before restore
        self._last_restore_pos: QPointF | None = None
        self._shift_held_in_erase: bool = False

        # Mask visualization overlay cache
        self._mask_overlay_image: QImage | None = None
        self._mask_overlay_version: int = -1  # mismatch forces first build
    ...
    def mouse_press(self, event: QMouseEvent, world_pos: QPointF, hex_coord: Hex) -> None:
        ...
        if self.mode == "erase":
            ...
            if ctrl and not shift:
                ...
                return
            # Shift+drag: restore stroke (paint opaque white back)
            if shift and not ctrl:
                map_bounds = self._project.grid_config.get_map_pixel_bounds()
                layer.ensure_mask(map_bounds)
                self._restore_pre_snapshot = layer.get_mask_snapshot()
                self._restore_stroke_active = True
                layer.begin_restore_stroke()
                self._last_restore_pos = world_pos
                layer.restore_at(wx, wy, self.erase_brush_size)
                return
            # Normal erase stroke
            map_bounds = self._project.grid_config.get_map_pixel_bounds()
            layer.ensure_mask(map_bounds)
            ...
            if self._pending_image_path:
                ...
                if self.snap_to_hex and self.placement_radius > 0:
                    ...
                    for h in hexes:
                        ...
                        cmd.execute()
                        compound.add(cmd)
                    self._command_stack.push_compound(compound)
                else:
    ...
    def mouse_move(self, event: QMouseEvent, world_pos: QPointF, hex_coord: Hex) -> None:
        ...
        # In erase mode always update (Ctrl/Alt are used for restore/size)
        if self.mode == "erase":
        ...
        if self.mode == "erase":
            ...
            self._shift_held_in_erase = shift_held
            if self._erase_size_dragging:
                ...
                return
            if self._restore_stroke_active and (
                ...
                if layer:
                    ...
                    if self._last_restore_pos is not None:
                        ...
                        dy = world_pos.y() - self._last_restore_pos.y()
                        if dx * dx + dy * dy < min_dist * min_dist:
                            return
                    layer.restore_at(world_pos.x(), world_pos.y(), self.erase_brush_size)
                    self._last_restore_pos = world_pos
                return
    ...
    def mouse_release(self, event: QMouseEvent, world_pos: QPointF, hex_coord: Hex) -> None:
        # Erase / restore stroke end: push undo command
        if self.mode == "erase" and event.button() == Qt.MouseButton.LeftButton:
            ...
            if self._restore_stroke_active:
                self._restore_stroke_active = False
                layer = self._get_active_asset_layer()
                if layer and self._restore_pre_snapshot is not None:
                    layer.end_erase_stroke()
                    post_snapshot = layer.get_mask_snapshot()
                    cmd = PaintMaskCommand(layer, self._restore_pre_snapshot, post_snapshot)
                    self._command_stack.push_compound(cmd)
                self._restore_pre_snapshot = None
                self._last_restore_pos = None
                return
    ...
    def key_release(self, event: QKeyEvent) -> None:
        # Erase mode: stop brush size drag when Ctrl is released
    ...
    def _build_mask_overlay(self, mask_image: QImage) -> QImage:
        """Build a semi-transparent red overlay showing erased regions."""
        w, h = mask_image.width(), mask_image.height()
        overlay = QImage(w, h, QImage.Format.Format_ARGB32_Premultiplied)
        overlay.fill(QColor(255, 0, 0, 80))
        p = QPainter(overlay)
    ...
    def paint_overlay(
        ...
        # Erase mode: show mask overlay + brush circle cursor
        if self.mode == "erase":
            layer = self._get_active_asset_layer()

            # Draw mask visualization overlay (red tint on erased areas)
            if layer is not None and layer.mask_image is not None:
                if (self._mask_overlay_image is None
                        or self._mask_overlay_version != layer._mask_version):
                    self._mask_overlay_image = self._build_mask_overlay(
                        layer.mask_image)
                    self._mask_overlay_version = layer._mask_version
                ox, oy = layer._mask_world_offset
                mw = layer.mask_image.width() / layer._mask_world_scale
                mh = layer.mask_image.height() / layer._mask_world_scale
                painter.save()
                painter.setRenderHint(
                    QPainter.RenderHint.SmoothPixmapTransform, False)
                painter.drawImage(QRectF(ox, oy, mw, mh),
                                  self._mask_overlay_image)
                painter.restore()

            # Brush cursor: green when restoring (Ctrl held), red when erasing
