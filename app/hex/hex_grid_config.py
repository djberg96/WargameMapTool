...
class HexGridConfig:
    ...
    center_dot_opacity: int = 100
    coord_opacity: int = 100

    # Megahex overlay
    megahex_opacity: int = 100
    ...
    # Grain overlay (film-grain noise over hex area, rendered after lighting tint)
    grain_enabled: bool = False
    grain_intensity: int = 20          # 0-100 (maps to alpha 0.0-1.0 at paint time)
    grain_scale: float = 1.0           # 1.0-10.0 (1.0 = 1px noise, 10.0 = coarse)
    grain_monochrome: bool = True      # True = grayscale noise, False = RGB color noise
    grain_seed: int = 42               # Reproducible random seed
    ...
    # Grain tile cache (invalidated when seed/scale/monochrome change)
    _grain_cache_key: tuple | None = field(default=None, repr=False, compare=False)
    _cached_grain_tile: object = field(default=None, repr=False, compare=False)
    ...
    def get_grain_tile(self):
        """Return a cached 256x256 ARGB32 noise tile for the grain overlay.

        The tile is regenerated when seed, scale, or monochrome changes.
        Intensity is NOT baked in — it is applied via painter.setOpacity() at
        render time so the tile can be reused when only the slider moves.
        """
        import numpy as np
        from PySide6.QtGui import QImage

        key = (self.grain_seed, self.grain_scale, self.grain_monochrome)
        if self._grain_cache_key == key and self._cached_grain_tile is not None:
            return self._cached_grain_tile

        rng = np.random.default_rng(self.grain_seed)

        # Base tile: scale=1 → 256px, scale=N → 256/N px (upscaled for coarser grain)
        base = max(1, int(round(256.0 / self.grain_scale)))

        if self.grain_monochrome:
            gray = rng.integers(0, 256, (base, base), dtype=np.uint8)
            r = g = b = gray
        else:
            r = rng.integers(0, 256, (base, base), dtype=np.uint8)
            g = rng.integers(0, 256, (base, base), dtype=np.uint8)
            b = rng.integers(0, 256, (base, base), dtype=np.uint8)

        # Alpha = 255 (opacity controlled at paint time)
        a = np.full((base, base), 255, dtype=np.uint8)

        # Pack as ARGB32 (Qt byte order: 0xAARRGGBB)
        pixels = (
        ...
        small_img = QImage(pixels_c.data, base, base, base * 4,
                           QImage.Format.Format_ARGB32).copy()

        # Scale up to 256x256 with NearestNeighbor for pixel-perfect coarse grain
        if base < 256:
            tile = small_img.scaled(
                256, 256,
    ...
    def serialize(self) -> dict:
        return {
            ...
            "center_dot_opacity": self.center_dot_opacity,
            "coord_opacity": self.coord_opacity,
            "megahex_opacity": self.megahex_opacity,
            ...
            "grain_monochrome": self.grain_monochrome,
    ...
    def deserialize(cls, data: dict) -> HexGridConfig:
        return cls(
            ...
            grid_opacity=data.get("grid_opacity", 100),
            center_dot_opacity=data.get("center_dot_opacity", 100),
            coord_opacity=data.get("coord_opacity", 100),
            megahex_opacity=data.get("megahex_opacity", 100),
            ...
            grain_enabled=data.get("grain_enabled", False),
            grain_intensity=data.get("grain_intensity", 20),
            grain_scale=float(data.get("grain_scale", 1.0)),
            grain_monochrome=data.get("grain_monochrome", True),
            grain_seed=data.get("grain_seed", 42),
