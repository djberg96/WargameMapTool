...
class HexGridRenderer:
    ...
    def paint(
        ...
        # Grid lines — use offscreen buffer when opacity < 100% to prevent
        # alpha accumulation on shared hex edges (drawn twice by adjacent hexes).
        grid_op = config.grid_opacity / 100.0
        if grid_op > 0.0:
            def _draw_grid(p: QPainter) -> None:
                if config.grid_style == "crossings":
                    self._draw_hex_crossings(p, visible_hexes, layout, config)
                else:
                    self._draw_hex_outlines(p, visible_hexes, layout, config)

            if grid_op < 1.0:
                self._paint_buffered(painter, grid_op, _draw_grid)
            else:
                _draw_grid(painter)

        # Megahexes (own opacity, independent of grid lines)
        if config.megahex_enabled:
            mega_op = config.megahex_opacity / 100.0
            if mega_op > 0.0:
                def _draw_mega(p: QPainter) -> None:
                    self._draw_megahexes(p, visible_hexes, layout, config)

                if mega_op < 1.0:
                    self._paint_buffered(painter, mega_op, _draw_mega)
                else:
                    _draw_mega(painter)

        # Center dots (one per hex center — no overlap, but buffer keeps
        # consistency if outline + fill dots would stack)
        if config.show_center_dots:
            dot_op = config.center_dot_opacity / 100.0
            if dot_op > 0.0:
                def _draw_dots(p: QPainter) -> None:
                    self._draw_center_dots(p, visible_hexes, layout, config, dot_overrides)

                if dot_op < 1.0:
                    self._paint_buffered(painter, dot_op, _draw_dots)
                else:
                    _draw_dots(painter)

        # Coordinates
        if config.show_coordinates:
            coord_op = config.coord_opacity / 100.0
            if coord_op > 0.0:
                def _draw_coords(p: QPainter) -> None:
                    self._draw_coordinates(p, visible_hexes, layout, config, coord_overrides)

                if coord_op < 1.0:
                    self._paint_buffered(painter, coord_op, _draw_coords)
                else:
                    _draw_coords(painter)
    ...
    def _paint_buffered(
        ...
        """Render *callback* into an offscreen buffer, then composite at *opacity*.

        This avoids alpha accumulation when overlapping geometry (e.g. shared
        hex edges) would be drawn more than once.
        """
        device = painter.device()
        buf = QImage(device.width(), device.height(),
                     QImage.Format.Format_ARGB32_Premultiplied)
        buf.fill(Qt.GlobalColor.transparent)

        bp = QPainter(buf)
        bp.setRenderHints(painter.renderHints())
        bp.setTransform(painter.transform())
        if painter.hasClipping():
            bp.setClipPath(painter.clipPath())
        callback(bp)
        bp.end()
        ...
        painter.setOpacity(base_op * opacity)
