...
<ul>
  ...
  <li><b>Opacity</b> — transparency of grid lines and megahexes (0–100 %).</li>
</ul>
...
<ul>
  ...
  <li><b>Opacity</b> — transparency of center dots (0–100 %).</li>
</ul>
...
<ul>
  ...
  <li><b>Opacity</b> — transparency of coordinate labels (0–100 %).</li>
</ul>
...
<ul>
  ...
  <li><b>Opacity</b> — megahex line opacity (0–100 %).</li>
</ul>

<h3>Paper Format (New Map only)</h3>
<p>Calculates the required grid dimensions (columns × rows) to fill a standard paper
format at the current hex size.</p>
<ul>
  <li><b>Format</b> — select a paper standard: ISO A0–A5, US Letter, Legal, Tabloid,
      or ANSI C/D/E.</li>
  <li><b>Orientation</b> — Portrait or Landscape (default: Landscape).</li>
  <li><b>Margin</b> — print margin in mm (0–50, default 10 mm). The available area
      for hexes is reduced by twice this value on each axis.</li>
  <li><b>Set Map to Paper Format</b> button — calculates the maximum grid that fits
      the available area and updates the Columns / Rows fields.</li>
</ul>
...
<table>
  ...
  <tr><td><code>Ctrl+Shift+I</code></td><td>Zoom In — zooms in by one step, anchored at screen center.</td></tr>
  <tr><td><code>Ctrl+Shift+O</code></td><td>Zoom Out — zooms out by one step, anchored at screen center.</td></tr>
</table>
...
<table>
  ...
  <tr><td><b>Create Random Map…</b></td><td>—</td>
      <td>Opens the random map generator dialog. Only available on a fresh map (single Fill layer).
          See <a href="page:dlg_random_map">Create Random Map</a>.</td></tr>
</table>
""")
...
<table>
  ...
  <tr><td><b>Zoom In</b></td><td><code>Ctrl+Shift+I</code></td>
      <td>Zooms in by one step, anchored at screen center.</td></tr>
  <tr><td><b>Zoom Out</b></td><td><code>Ctrl+Shift+O</code></td>
      <td>Zooms out by one step, anchored at screen center.</td></tr>
  ...
  <tr><td><b>Global Lighting…</b></td><td><code>Ctrl+L</code></td>
      <td>Opens the Global Lighting dialog (non-modal). Applies a semi-transparent color
          tint over the map area. Also includes a configurable <b>grain</b> (film-noise)
          overlay with intensity, scale, monochrome, and seed controls. Both effects
          are included in exports.</td></tr>
</table>

<h3>Performance Dialog</h3>
<p><b>View &gt; Performance...</b> opens a settings dialog with three pages:</p>

<h4>Cache Delay  (<code>Ctrl+Shift+D</code> to cycle)</h4>
<p>Controls how long the editor waits after an edit before rebuilding the full layer
pixmap cache. During the delay, the canvas renders directly with viewport culling
(fast, bounded by screen resolution). The cache is rebuilt once editing pauses.</p>
<ul>
  <li><b>None</b> — always rebuild immediately (best for small maps).</li>
  <li><b>50 ms / 100 ms</b> — short delay (good for small to medium maps).</li>
  <li><b>200 ms</b> (default) — balanced (good for large maps, 30&times;30+).</li>
  <li><b>500 ms</b> — long delay (best for very large maps or heavy effects).</li>
</ul>

<h4>Zoom Settle Delay  (<code>Ctrl+Shift+Z</code> to cycle)</h4>
<p>Controls how long the editor waits after zooming before rebuilding the layer
cache. During the delay the canvas uses the existing (possibly stale) cache.
The cache is rebuilt once zooming stops.</p>
<ul>
  <li><b>50 ms / 100 ms</b> — short delay (responsive, but may rebuild frequently).</li>
  <li><b>200 ms</b> (default) — balanced.</li>
  <li><b>500 ms / 1000 ms</b> — long delay (best for heavy maps with many layers).</li>
</ul>

<h4>Render Quality  (<code>Ctrl+Shift+Q</code> to toggle)</h4>
<ul>
...
<h4>Sharp Lines  (<code>Ctrl+Shift+L</code> to toggle)</h4>
<p>Controls whether the <b>Hexside</b>, <b>Path</b>, <b>Freeform Path</b>, and
<b>Border</b> layers use screen-resolution caches for their rendered output.</p>
<ul>
  <li><b>Off</b> (default) — line layers use world-resolution caches (same as the
      global Performance mode). Fast, low memory, but lines may appear slightly
      blurry when zoomed in.</li>
  <li><b>On</b> — line layers use screen-resolution caches, so edges and paths are
      rendered sharply at every zoom level. Costs more memory and triggers a cache
      rebuild after each zoom change, but has no effect on non-line layers.</li>
</ul>
<p>This setting is independent of the global Render Quality toggle and applies only
to line-type layers. It is useful when you need crisp borders and paths without
switching the entire map to Quality mode.</p>

<h4>Edge Bleeding Quality  (<code>Ctrl+Shift+B</code> to toggle)</h4>
<p>Controls the resolution of the distance field used for the <b>Edge Bleeding</b>
effect on Draw Layer channels.</p>
<ul>
  <li><b>Performance</b> (default) — downsamples the mask 4&times; before computing the
      distance field (16&times; fewer pixels). Much faster, with a slight softness at
      the transition boundary.</li>
  <li><b>Quality</b> — computes the distance field at full mask resolution. Sharp
      transitions, but slower after each stroke.</li>
</ul>
...
and <b>Edge Bleeding</b> (Draw Layer) effects are suppressed — the underlying
layer content is still drawn normally.
<b>Outline</b> (Draw Layer only) is not affected.</p>
<p>This is useful when working with many layers that have expensive effects enabled,
as it can significantly reduce rendering time and eliminate lag.</p>
<p>Exports always include all effects regardless of this setting.</p>
<p>All performance settings are persisted across sessions. <b>Exports always force
all settings to their highest quality values</b> (Quality rendering, Sharp Lines ON,
Edge Bleeding Quality, full GFX effects) regardless of the current display mode.</p>
""")

_DOCS["menu_help"] = _page("Help Menu", """
<table>
  <tr><th>Item</th><th>Shortcut</th><th>Description</th></tr>
  <tr><td><b>Documentation…</b></td><td><code>F2</code></td>
...
<p>Paints hex cells with solid colors, textures, custom dot colors, hex-edge
fills, or stipple (edge scatter) effects. Multiple Fill layers can be stacked
for visual effects.</p>
<p><b>Hotkey:</b> <code>F</code></p>

<h3>Tool Options – Mode</h3>
<p>Select one of five target modes:</p>
<ul>
  ...
  <li><b>Stipple</b> — renders gradient bands along outer edges of painted hex
      regions, creating organic terrain transitions (e.g. hill edges bleeding
      into plains). Supports both flat color and tiled texture fill.
      Supports nested zones via the <em>Priority</em> setting.</li>
</ul>

<h3>Tool Options – Paint Mode (Hex and Stipple modes)</h3>
<p>Toggle between <b>Color</b> and <b>Texture</b>. In Hex mode this switches
between color fill and texture fill. In Stipple mode it switches between
color gradient bands and texture gradient bands.</p>
...
<h3>Tool Options – Stipple (Stipple mode only)</h3>
<p>Use the <b>Paint Mode</b> toggle (Color / Texture) to switch between
flat-color and tiled-texture gradient bands.</p>
<ul>
  <li><b>Color mode</b> — fills gradient bands with a flat color.
      <ul><li><b>Color button</b> — opens the color picker for the gradient.</li></ul>
  </li>
  <li><b>Texture mode</b> — fills gradient bands with a tiled texture from the
      texture library. Uses mask-based compositing (alpha gradient as mask,
      texture tiled underneath).
      <ul>
        <li><b>Game / Category / Search</b> — filter the texture thumbnail grid.</li>
        <li><b>Manager…</b> — opens the Texture Manager.</li>
        <li><b>Expand</b> — opens a larger texture browser sidebar.</li>
        <li><b>Zoom</b> — scale the texture tile (0.05x – 5.0x).</li>
        <li><b>Rotation</b> — rotate the texture tile (0° – 359°).</li>
      </ul>
  </li>
  <li><b>Spread (mm)</b> — how far the gradient extends outward from the hex edge
      into neighboring hexes.</li>
  <li><b>Inset (mm)</b> — how far the gradient extends inward into the hex
      itself. At 0 (default) the color/texture only bleeds outward; higher
      values make the gradient also fade into the hex interior.</li>
  <li><b>Inset Falloff</b> — percentage of the inset distance that has an
      alpha transition (0–100%). At 100% the entire inset is a gradient;
      at 50% only the inner half fades while the outer half stays solid.</li>
  <li><b>Falloff</b> — percentage of the spread distance that has an alpha
      transition (0–100%). At 100% the entire spread is a gradient; at 50%
      only the outer half fades while the inner half stays solid.</li>
  <li><b>Jitter</b> — noise amplitude (0–1). Controls how irregular both the
      outer and inner edges of the gradient band appear. 0 = smooth edges,
      1 = very ragged, organic-looking boundary.</li>
  <li><b>Priority</b> — nesting level (0–10). Higher-priority zones extend
      their gradient into lower-priority zones, enabling nested terrain
      transitions (e.g. a priority-1 forest zone inside a priority-0 field
      zone). Two zones at the same priority never bleed into each other.</li>
</ul>
...
<p>While in Erase Mode, erased areas are highlighted with a <b>semi-transparent
red overlay</b> so you can see exactly what has been erased. The overlay
disappears when you leave Erase Mode.</p>
<ul>
...
<table>
  <tr><th>Input</th><th>Action</th></tr>
  <tr><td>Left Drag</td><td>Erase (paint transparent into the mask). Cursor: <b>red</b> dashed circle.</td></tr>
  <tr><td><code>Shift</code> + Left Drag</td><td>Restore (paint erased areas back to visible). Cursor: <b>green</b> dashed circle.</td></tr>
...
<table>
  ...
  <tr><td><code>F4</code></td><td>Edit text content.</td></tr>
...
<h3>Tool Options – Edge Bleeding</h3>
<p>Per-channel gradient transition at the edges of painted areas (color mode only).
Creates a natural fade from the main channel color to a second color at the mask
boundary — useful for river banks, terrain transitions, etc.</p>
<ul>
  <li><b>Enable</b> checkbox — toggle edge bleeding on/off.</li>
  <li><b>Color</b> button — the edge/transition color (appears at the outer rim).</li>
  <li><b>Distance</b> slider — how far (in mask pixels) the gradient extends
      inward from the painted edge. Larger values = wider transition.</li>
  <li><b>Organic</b> slider — controls the transition style.
      At <b>0</b> the boundary between the two colors is a hard, binary edge.
      As you increase the value, the transition becomes softer and more organic
      with noise-based randomness (1.0 = maximum organic randomness).</li>
</ul>
<p>The effect is computed after each stroke and is visible once you release the
mouse button. The Distance and Organic sliders use a debounce timer (300 ms)
to avoid expensive recomputation on every slider tick.</p>
<p>You can switch between <b>Performance</b> (downsampled distance field, faster)
and <b>Quality</b> (full-resolution) mode via <code>Ctrl+Shift+B</code> or in
<em>View &gt; Performance &gt; Render Quality &gt; Edge Bleeding</em>.
Exports always use Quality mode regardless of this setting.</p>
...
<ul>
  ...
  <li><b>Fill Selection with Color</b> — fills all selected pixels with the
      current Paint Brush color. The selection remains active so you can
      undo, change color, and fill again.</li>
</ul>
...
<ul>
  <li><b>↩ Undo</b> button or <code>Ctrl+Z</code> — undoes the last destructive
      operation (paint stroke, posterize, delete, fill, or outline). Up to 10 steps.</li>
</ul>
...
_DOCS["dlg_random_map"] = _page("Create Random Map", """
<p>Opened via <b>Edit &gt; Create Random Map…</b>. Generates a procedural random
terrain map on the current grid. Only available when the project contains exactly
one Fill layer and no other layers (i.e. a freshly created map).</p>

<h3>Map Type</h3>
<ul>
  <li><b>Continental</b> — Mostly land; water only appears in low-elevation areas.</li>
  <li><b>Coast</b> — One side of the map is ocean. Choose the coast side (North/South/East/West).</li>
  <li><b>Island</b> — Land mass in the centre surrounded by water.</li>
</ul>

<h3>Terrain Sliders</h3>
<ul>
  <li><b>Water</b> — Percentage of hexes that become water (0–100).</li>
  <li><b>Mountains</b> — Percentage of land that becomes hills (split into Hill 1/2/3 tiers).</li>
  <li><b>Forest</b> — Percentage of flat ground that becomes forest.</li>
  <li><b>Fill Edges</b> — Checkbox: place hexside lines at terrain transitions,
      coloured by the lower terrain. Creates an <b>Edges</b> hexside layer.</li>
</ul>

<h3>Terrain Fills</h3>
<p>Select a colour palette and assign a palette colour to each terrain role
(Ground, Hill 1–3, Water, Forest). The dialog auto-assigns colours by matching
palette colour names (e.g. "Hill_1" maps to Hill 1). Override any assignment
by clicking the colour swatch.</p>
<p>Optionally, assign a <b>texture</b> to any terrain role by clicking its texture
button (shows "None" by default). A texture picker dialog opens where you can
browse and filter the texture library. When a texture is set for a role, those
hexes will receive a texture fill instead of a colour fill. Right-click the
texture button to clear it (revert to colour). Use the <b>Tex Zoom</b> slider
to control texture scale (0.1×–5.0×).</p>

<h3>Seed</h3>
<p>The random seed determines the noise pattern. Click <b>Randomize</b> for a new seed
(also refreshes the preview), or enter a specific seed to reproduce a map.</p>

<h3>Workflow</h3>
<ol>
  <li>Configure map type, sliders, fills (colours/textures).</li>
  <li>Click <b>Randomize</b> to generate previews with different seeds.</li>
  <li>Adjust settings until satisfied.</li>
  <li>Click <b>Apply</b> to write the terrain to the map. The dialog closes.</li>
  <li>Use <b>Ctrl+Z</b> to undo if the result is not satisfactory.</li>
</ol>
""")
...
_TREE: list[tuple[str, list[tuple[str, str]]]] = [
    ...
    ("Dialogs", [
        ...
        ("dlg_random_map",    "Create Random Map"),
