...
class DrawOptions:
    ...
    def __init__(self, dock: ToolOptionsPanel) -> None:
        ...
        # Edge bleeding controls
        self._edge_group: QGroupBox | None = None
        self._edge_enable_cb: QCheckBox | None = None
        self._edge_container: QWidget | None = None
        self._edge_color_btn: QPushButton | None = None
        self._edge_color_hex: str = "#aaccee"
        self._edge_dist_slider: QSlider | None = None
        self._edge_dist_spin: QDoubleSpinBox | None = None
        self._edge_noise_slider: QSlider | None = None
        self._edge_noise_spin: QDoubleSpinBox | None = None

        # Debounce timer for edge bleeding sliders (300ms delay)
        self._edge_debounce = QTimer()
        self._edge_debounce.setSingleShot(True)
        self._edge_debounce.setInterval(300)
        self._edge_debounce.timeout.connect(self._edge_debounce_fire)
        self._edge_pending_changes: dict[str, object] = {}
    ...
    def create(self, tool: DrawTool) -> QWidget:
        ...
        # ===== Edge Bleeding group (per-channel, color mode only) =====
        self._edge_group = QGroupBox("Edge Bleeding")
        edge_gl = QVBoxLayout(self._edge_group)
        edge_gl.setContentsMargins(6, 4, 6, 4)

        self._edge_enable_cb = QCheckBox("Enable")
        self._edge_enable_cb.setChecked(False)
        self._edge_enable_cb.toggled.connect(self._on_edge_enable_toggled)
        edge_gl.addWidget(self._edge_enable_cb)

        self._edge_container = QWidget()
        ec_layout = QVBoxLayout(self._edge_container)
        ...
        update_color_btn(self._edge_color_btn, QColor(self._edge_color_hex))
        self._edge_color_btn.clicked.connect(self._on_edge_color_pick)
        ec_row.addWidget(self._edge_color_btn)
        ec_row.addStretch()
        ec_layout.addLayout(ec_row)

        ed_row = QHBoxLayout()
        ed_row.addWidget(QLabel("Distance:"))
        self._edge_dist_slider = QSlider(Qt.Orientation.Horizontal)
        self._edge_dist_slider.setRange(1, 100)
        self._edge_dist_slider.setValue(20)
        self._edge_dist_slider.valueChanged.connect(self._on_edge_dist_slider)
        ed_row.addWidget(self._edge_dist_slider, 1)
        self._edge_dist_spin = QDoubleSpinBox()
        self._edge_dist_spin.setRange(1.0, 100.0)
        self._edge_dist_spin.setSingleStep(1.0)
        self._edge_dist_spin.setDecimals(0)
        self._edge_dist_spin.setValue(20.0)
        self._edge_dist_spin.valueChanged.connect(self._on_edge_dist_spin)
        ed_row.addWidget(self._edge_dist_spin)
        ec_layout.addLayout(ed_row)

        en_row = QHBoxLayout()
        en_row.addWidget(QLabel("Organic:"))
        self._edge_noise_slider = QSlider(Qt.Orientation.Horizontal)
        self._edge_noise_slider.setRange(0, 100)
        self._edge_noise_slider.setValue(30)
        self._edge_noise_slider.valueChanged.connect(self._on_edge_noise_slider)
        en_row.addWidget(self._edge_noise_slider, 1)
        self._edge_noise_spin = QDoubleSpinBox()
        self._edge_noise_spin.setRange(0.0, 1.0)
        self._edge_noise_spin.setSingleStep(0.05)
        self._edge_noise_spin.setDecimals(2)
        self._edge_noise_spin.setValue(0.30)
        self._edge_noise_spin.valueChanged.connect(self._on_edge_noise_spin)
        en_row.addWidget(self._edge_noise_spin)
        ec_layout.addLayout(en_row)

        edge_gl.addWidget(self._edge_container)
        self._edge_container.setEnabled(False)
    ...
    def _update_channel_content(self) -> None:
        ...
        try:
            ...
            # Edge bleeding controls (color mode only)
            if self._edge_group is not None:
                self._edge_group.setVisible(not is_texture)
                edge_enabled = bool(ch.edge_color)
                self._edge_enable_cb.blockSignals(True)
                self._edge_enable_cb.setChecked(edge_enabled)
                self._edge_enable_cb.blockSignals(False)
                if self._edge_container:
                    self._edge_container.setEnabled(edge_enabled)
                if edge_enabled and self._edge_color_btn:
                    update_color_btn(self._edge_color_btn, QColor(ch.edge_color))
                    self._edge_color_hex = ch.edge_color
                if self._edge_dist_slider:
                    self._edge_dist_slider.blockSignals(True)
                    self._edge_dist_slider.setValue(int(ch.edge_distance))
                    self._edge_dist_slider.blockSignals(False)
                if self._edge_dist_spin:
                    self._edge_dist_spin.blockSignals(True)
                    self._edge_dist_spin.setValue(ch.edge_distance)
                    self._edge_dist_spin.blockSignals(False)
                if self._edge_noise_slider:
                    self._edge_noise_slider.blockSignals(True)
                    self._edge_noise_slider.setValue(int(ch.edge_noise * 100))
                    self._edge_noise_slider.blockSignals(False)
                if self._edge_noise_spin:
                    self._edge_noise_spin.blockSignals(True)
                    self._edge_noise_spin.setValue(ch.edge_noise)
                    self._edge_noise_spin.blockSignals(False)
    ...
    def _on_ch_mode_changed(self, btn_id: int, checked: bool) -> None:
        ...
        if btn_id == 0:
            ...
            if ch and ch.texture_id:
                ...
                if self._edge_group:
                    self._edge_group.setVisible(True)
        else:
    ...
    def _on_edge_enable_toggled(self, checked: bool) -> None:
        if self._updating_ui:
            return
        ch = self._get_active_channel()
        if ch is None:
            return
        if checked:
            self._apply_channel_edit(edge_color=self._edge_color_hex)
        else:
            self._apply_channel_edit(edge_color="")
        if self._edge_container:
            self._edge_container.setEnabled(checked)

    def _on_edge_color_pick(self) -> None:
        ch = self._get_active_channel()
        if ch is None:
            return
        initial = QColor(ch.edge_color) if ch.edge_color else QColor(self._edge_color_hex)
        color = QColorDialog.getColor(initial, self.dock.window(), "Edge Color")
        if color.isValid():
            self._edge_color_hex = color.name()
            update_color_btn(self._edge_color_btn, color)
            self._apply_channel_edit(edge_color=color.name())

    def _on_edge_dist_slider(self, val: int) -> None:
        if self._updating_ui:
            return
        if self._edge_dist_spin:
            self._edge_dist_spin.blockSignals(True)
            self._edge_dist_spin.setValue(float(val))
            self._edge_dist_spin.blockSignals(False)
        self._edge_pending_changes["edge_distance"] = float(val)
        self._edge_debounce.start()

    def _on_edge_dist_spin(self, val: float) -> None:
        if self._updating_ui:
            return
        if self._edge_dist_slider:
            self._edge_dist_slider.blockSignals(True)
            self._edge_dist_slider.setValue(int(val))
            self._edge_dist_slider.blockSignals(False)
        self._edge_pending_changes["edge_distance"] = val
        self._edge_debounce.start()

    def _on_edge_noise_slider(self, val: int) -> None:
        if self._updating_ui:
            return
        v = val / 100.0
        if self._edge_noise_spin:
            self._edge_noise_spin.blockSignals(True)
            self._edge_noise_spin.setValue(v)
            self._edge_noise_spin.blockSignals(False)
        self._edge_pending_changes["edge_noise"] = v
        self._edge_debounce.start()

    def _on_edge_noise_spin(self, val: float) -> None:
        if self._updating_ui:
            return
        if self._edge_noise_slider:
            self._edge_noise_slider.blockSignals(True)
            self._edge_noise_slider.setValue(int(val * 100))
            self._edge_noise_slider.blockSignals(False)
        self._edge_pending_changes["edge_noise"] = val
        self._edge_debounce.start()

    def _edge_debounce_fire(self) -> None:
        """Apply accumulated edge bleeding slider changes after debounce."""
        if not self._edge_pending_changes:
            return
        changes = self._edge_pending_changes.copy()
        self._edge_pending_changes.clear()
        self._apply_channel_edit(**changes)
