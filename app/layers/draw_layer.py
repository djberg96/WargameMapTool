...
# ---------------------------------------------------------------------------
# Edge bleeding helpers (pure numpy, no external deps)
# ---------------------------------------------------------------------------

# Quality mode for edge bleeding computation.
# False (default) = Performance: downsample mask before distance field (fast).
# True            = Quality: full-resolution distance field (sharp, slow).
# Forced to True during export via set_edge_bleed_quality_mode().
_edge_bleed_quality: bool = False

# Downsample factor in performance mode (4 = 1/4 resolution = 16× fewer pixels).
_PERF_DOWNSAMPLE = 4


def set_edge_bleed_quality_mode(enabled: bool) -> None:
    """Set edge bleeding render quality (affects all DrawLayer instances)."""
    global _edge_bleed_quality
    _edge_bleed_quality = enabled

def _chamfer_distance_inner(mask_alpha: np.ndarray, max_dist: int) -> np.ndarray:
    """Approximate inner distance from mask edge using iterative 4-neighbor erosion.

    mask_alpha: 2D uint8 array (h, w), values 0-255.
    max_dist:   Cap on iterations for performance.

    Returns 2D float32 array:
      0.0 at non-painted pixels and at the painted boundary,
      increasing inward toward the center of painted areas,
      capped at max_dist.
    """
    binary = mask_alpha > 0  # True = painted
    h, w = binary.shape
    if h < 3 or w < 3:
    ...
    for k in range(1, max_dist + 1):
        # Erode: pixel stays True only if all 4-connected neighbors are True
        ...
        # Pixels removed in this step have distance k
        removed = current & ~eroded
        dist[removed] = float(k)

        current = eroded
        if not inner.any():
            break

    # Pixels still remaining after all iterations
    dist[current] = float(max_dist)
    return dist


def _value_noise_2d(h: int, w: int, seed: int, scale: float = 1.0) -> np.ndarray:
    """Generate seeded 2D value noise in range [0, 1].

    Uses 3 octaves of bilinear-interpolated random grids.
    scale controls grain size: 1.0 = coarse (~32px cells), higher = finer.
    """
    rng = np.random.default_rng(seed)
    result = np.zeros((h, w), dtype=np.float32)
    amplitude = 1.0
    total_amplitude = 0.0
    cell_size = max(4, int(32 / max(0.1, scale)))

    for _ in range(3):
        grid_h = max(2, h // cell_size + 2)
        grid_w = max(2, w // cell_size + 2)
        grid = rng.random((grid_h, grid_w), dtype=np.float32)

        # Pixel positions in grid space
        yy = np.arange(h, dtype=np.float32) / cell_size
        xx = np.arange(w, dtype=np.float32) / cell_size
        y0 = np.clip(yy.astype(np.intp), 0, grid_h - 2)
        x0 = np.clip(xx.astype(np.intp), 0, grid_w - 2)
        fy = yy - y0.astype(np.float32)
        fx = xx - x0.astype(np.float32)

        # Bilinear interpolation via broadcasting
        fy2d = fy[:, np.newaxis]
        fx2d = fx[np.newaxis, :]
        y0_2d = y0[:, np.newaxis]
        x0_2d = x0[np.newaxis, :]
        interp = (
            grid[y0_2d, x0_2d] * (1 - fy2d) * (1 - fx2d)
            + grid[y0_2d + 1, x0_2d] * fy2d * (1 - fx2d)
            + grid[y0_2d, x0_2d + 1] * (1 - fy2d) * fx2d
            + grid[y0_2d + 1, x0_2d + 1] * fy2d * fx2d
        )
...
class DrawLayer(Layer):
    ...
    def __init__(self, name: str = "Draw"):
        ...
        # Per-channel edge-bleeding cache at mask resolution.
        # Keyed by channel.id → (params_key, QImage).
        # Survives pan/zoom (viewport-independent); invalidated by
        # mark_channel_dirty() and mark_dirty().
        self._edge_bleed_cache: dict[str, tuple[tuple, QImage]] = {}
    ...
    def mark_channel_dirty(self, channel_id: str) -> None:
        ...
        self._edge_bleed_cache.pop(channel_id, None)
        self._outline_cache = None
        self._shadow_cache = None
    ...
    def _calculate_bounds(self) -> QRectF:
        ...
        if gfx and self.shadow_enabled:
            margin = max(
                margin,
                self.shadow_distance + self.shadow_size + 2.0,
            )
        if gfx and self.bevel_enabled and self.bevel_type == "outer":
    ...
    def _render_channel_to_image(
        ...
        # Edge bleeding path (color mode only, respects Visible GFX toggle)
        if ch.edge_color and not ch.texture_id and self._gfx_effects_enabled:
            return self._render_channel_with_edge_bleeding(
                ch, composite_bounds, w, h, scale_x, scale_y
            )
    ...
    def _render_channel_with_edge_bleeding(
        self,
        ch: DrawChannel,
        composite_bounds: QRectF,
        w: int,
        ...
        """Render channel with edge-to-center color gradient transition.

        The expensive distance-field + noise computation runs at mask resolution
        and is cached in _edge_bleed_cache.  Only the cheap QPainter scale/draw
        into the composite-sized output runs on every viewport change.
        """
        mask_img = self._get_edge_bleed_image(ch)
        if mask_img is None:
            return None

        # Place cached mask-resolution image into composite-sized result
        ...
        p.translate(ox, oy)
        p.scale(sx, sy)
        p.drawImage(QPointF(0.0, 0.0), mask_img)
        p.end()

        return result

    def _get_edge_bleed_image(self, ch: DrawChannel) -> QImage | None:
        """Return the mask-resolution blended image, from cache if possible.

        In quality mode the distance field is computed at full mask resolution.
        In performance mode the mask is downsampled first (16× fewer pixels),
        then upscaled — much faster with a slight softness trade-off.

        Cache key includes the quality flag so switching modes rebuilds.
        Invalidated by mark_channel_dirty() / mark_dirty() when mask or
        params change.
        """
        mask = ch.mask_image
        if mask is None:
        ...
        quality = _edge_bleed_quality
        cache_key = (
            ch.edge_color, ch.edge_distance, ch.edge_noise,
            ch.color, mask_w, mask_h, quality,
        )

        cached = self._edge_bleed_cache.get(ch.id)
        if cached is not None and cached[0] == cache_key:
            return cached[1]

        # --- Expensive path: compute distance field + noise + color blend ---
        ptr = mask.constBits()
        arr = np.frombuffer(ptr, dtype=np.uint8).reshape(mask_h, mask_w, 4).copy()
        mask_alpha_full = arr[:, :, 3]

        edge_dist_px = max(1.0, ch.edge_distance)

        # Performance mode: downsample mask before computing distance field
        ds = 1 if quality else _PERF_DOWNSAMPLE
        if ds > 1 and mask_h > ds * 4 and mask_w > ds * 4:
            # Block-average downsample via reshape
            sh = (mask_h // ds) * ds
            sw = (mask_w // ds) * ds
            cropped = mask_alpha_full[:sh, :sw]
            small = cropped.reshape(sh // ds, ds, sw // ds, ds).max(axis=(1, 3))
            small_dist_px = edge_dist_px / ds
            max_iter = min(int(math.ceil(small_dist_px)), 200)
            dist_small = _chamfer_distance_inner(small, max_iter)
            dist_small = np.clip(dist_small, 0.0, small_dist_px)

            if ch.edge_noise > 0.001:
                # Organic > 0: smooth gradient with noise
                t_small = dist_small / small_dist_px
                seed = hash(ch.id) & 0x7FFFFFFF
                noise = _value_noise_2d(small.shape[0], small.shape[1], seed)
                noise = noise * 2.0 - 1.0
                t_small = np.clip(t_small + noise * ch.edge_noise, 0.0, 1.0)
            else:
                # Organic == 0: hard binary boundary
                t_small = (dist_small >= small_dist_px).astype(np.float32)

            t_small[small == 0] = 0.0

            # Upscale t back to full resolution via nearest-neighbor repeat
            t = t_small.repeat(ds, axis=0).repeat(ds, axis=1)
            # Pad back to original size if mask wasn't evenly divisible
            if t.shape[0] < mask_h or t.shape[1] < mask_w:
                t_full = np.zeros((mask_h, mask_w), dtype=np.float32)
                t_full[:t.shape[0], :t.shape[1]] = t
                t = t_full
            mask_alpha = mask_alpha_full
        else:
            # Quality mode (or mask too small to downsample)
            mask_alpha = mask_alpha_full
            max_iter = min(int(math.ceil(edge_dist_px)), 200)
            dist_field = _chamfer_distance_inner(mask_alpha, max_iter)
            dist_field = np.clip(dist_field, 0.0, edge_dist_px)

            if ch.edge_noise > 0.001:
                # Organic > 0: smooth gradient with noise
                t = dist_field / edge_dist_px
                seed = hash(ch.id) & 0x7FFFFFFF
                noise = _value_noise_2d(mask_h, mask_w, seed)
                noise = noise * 2.0 - 1.0
                t = np.clip(t + noise * ch.edge_noise, 0.0, 1.0)
            else:
                # Organic == 0: hard binary boundary
                t = (dist_field >= edge_dist_px).astype(np.float32)

            t[mask_alpha == 0] = 0.0

        # Color blend: edge_color → main_color
        main_c = QColor(ch.color)
        edge_c = QColor(ch.edge_color)
        mr, mg, mb = main_c.red(), main_c.green(), main_c.blue()
        er, eg, eb = edge_c.red(), edge_c.green(), edge_c.blue()

        t_f = t.astype(np.float32)
        r = np.clip(er + (mr - er) * t_f, 0, 255).astype(np.uint8)
        g = np.clip(eg + (mg - eg) * t_f, 0, 255).astype(np.uint8)
        b = np.clip(eb + (mb - eb) * t_f, 0, 255).astype(np.uint8)
        a = mask_alpha

        # Pack BGRA premultiplied
        blended = np.zeros((mask_h, mask_w, 4), dtype=np.uint8)
        a_f = a.astype(np.float32) / 255.0
        blended[:, :, 0] = np.clip(b.astype(np.float32) * a_f, 0, 255).astype(np.uint8)
        blended[:, :, 1] = np.clip(g.astype(np.float32) * a_f, 0, 255).astype(np.uint8)
        blended[:, :, 2] = np.clip(r.astype(np.float32) * a_f, 0, 255).astype(np.uint8)
        blended[:, :, 3] = a

        blended_c = np.ascontiguousarray(blended)
        result_img = QImage(
            blended_c.data, mask_w, mask_h, mask_w * 4,
            QImage.Format.Format_ARGB32_Premultiplied,
        ).copy()

        self._edge_bleed_cache[ch.id] = (cache_key, result_img)
        return result_img
    ...
    def _draw_composite_screen(
        ...
        has_effects = (
            ...
            or (gfx and self.shadow_enabled)
            or (gfx and self.bevel_enabled)
            or (gfx and self.structure_enabled)
        )
        ...
        if gfx and self.structure_enabled and self.structure_texture_id:
        ...
        if gfx and self.bevel_enabled and self.bevel_type == "inner":
            avg_zoom = (zoom_x + zoom_y) * 0.5
