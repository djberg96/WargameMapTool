...
class BorderLayer(Layer):
    ...
    def remove_border(self, obj: BorderObject) -> None:
        self._path_cache.pop(obj.edge_key(), None)
    ...
    def hit_test(
        ...
        for obj in self.borders.values():
            path = self._get_cached_border_path(layout, obj)
            if path.isEmpty():
                continue
    ...
    def _make_outline_pen(
        ...
        if line_type == "dotted":
            ...
            pen.setCapStyle(cap)
            pen.setJoinStyle(Qt.PenJoinStyle.RoundJoin)
            if element_size <= 0 or gap_size <= 0:
                return pen
    ...
    def _get_cached_border_path(self, layout: Layout, obj: BorderObject) -> QPainterPath:
